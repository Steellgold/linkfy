...
<div class="fixed bottom-1 right-2">
  {#each $toasts as toast}
    <div class="w-50 sm:w-100 my-3" transition:fly={{ x: 200, duration: 500 }}>
      <div
        id="toast-simple"
        class="flex w-full max-w-xs items-center space-x-4 divide-x rounded-lg p-4 shadow {colors[
          toast[1]
        ]} space-x divide-gray-700 bg-gray-800"
        role="alert"
      >
        {#if toast[1] === "danger"}
          <IconAlertTriangle />
        {:else if toast[1] === "success"}
          <IconCheck />
        {:else if toast[1] === "warning"}
          <IconTrafficCone />
        {:else if toast[1] === "info"}
          <IconInfoCircle />
        {/if}
