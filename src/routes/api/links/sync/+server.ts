...
export async function GET({ request, getClientAddress }: RequestEvent): Promise<Response> {
  ...
  if (!userId || !visitorId) return new Response("Bad Request: You must provide a userId and a visitorId", { status: 400 });
  ...
  if (!links) return new Response("Internal Server Error: Failed to synchronize links", { status: 500 });
